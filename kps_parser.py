...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_path', type=str, default='kps_input')
    parser.add_argument('--out_path', type=str, default='kps_output')
    args = parser.parse_args()
    img_path = args.img_path
    out_path = Path(args.out_path)
    print("[INFO]Pose estimation.")

    img_file = "/shared/artem/human_parser/input/ars.jpg"
    #
    start = time.time()
    modelpath = "/shared/artem/kps_parser"
    ...
    for img_file in Path(img_path).glob('*.jpg'):
        ...
        res_points, confs = pose_model.predict(img_file)
        print("[INFO]Model predicts time: ", time.time() - start)
        pose_model.vis_pose(img_file, res_points)
        res_dict =  {"version": 1.0, "people": [{"face_keypoints": [], "pose_keypoints": [], "hand_right_keypoints": [], "hand_left_keypoints": []}]}

        for i in range(len(res_points)):
            if res_points[i] is not None:
                res_dict["people"][0]["pose_keypoints"].append(res_points[i][0])
                res_dict["people"][0]["pose_keypoints"].append(res_points[i][1])
                res_dict["people"][0]["pose_keypoints"].append(confs[i])
            else:
                res_dict["people"][0]["pose_keypoints"].append(0)
                res_dict["people"][0]["pose_keypoints"].append(0)
                res_dict["people"][0]["pose_keypoints"].append(confs[i])
        print(res_dict)
        with open(out_path.joinpath(img_file[:-4] + '_keypoints' + '.json'), 'w',1) as wf:
            json.dump(res_dict, wf)
